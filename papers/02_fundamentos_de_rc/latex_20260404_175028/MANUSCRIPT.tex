...
Em muitos textos de RC, a equação do reservatório aparece pronta, em uma unica linha. Para quem esta vendo o tema pela primeira vez, isso costuma ser brusco demais. Aqui vamos fazer o caminho inverso: primeiro identificamos os ingredientes da atualizacao, depois construimos o estado candidato, e só ao final escrevemos a equação completa.

\subsubsection{3.1 Os ingredientes da atualizacao}\label{31-os-ingredientes-da-atualizacao}

Em cada instante \(t\), o reservatório recebe uma entrada \(u_t\) e mantem internamente um estado \(x_{t-1}\) vindo do passo anterior. O objetivo da atualizacao e responder a pergunta:
...
O primeiro passo e formar uma ativacao bruta, que ainda não é o novo estado final. Chamaremos essa quantidade de pre-ativacao:

\[
z_t = W_{res} x_{t-1} + W_{in} u_t + b.
\]

Essa expressao deve ser lida com calma:
...
O vetor \(z_t\) representa, portanto, a resposta linear bruta do sistema antes de qualquer limitacao ou filtragem.

\subsubsection{3.3 Segundo passo: obter o estado candidato}\label{33-segundo-passo-obter-o-estado-candidato}

Se usassemos diretamente \(z_t\) como novo estado, o modelo permaneceria linear demais e os valores internos poderiam crescer sem controle. Por isso, aplicamos uma não linearidade componente a componente:
...
O vetor \(\widetilde{x}_t\) e chamado aqui de \textbf{estado candidato}. Ele representa a resposta que o reservatório produziria se reagisse imediatamente ao passo atual. A função \(\tanh(\cdot)\) comprime os valores para o intervalo \((-1,1)\), introduz não linearidade e ajuda a manter a dinâmica interna em uma faixa estável.
...
\tightlist
\item
  \(z_t\) é a ativacao bruta;
\item
  \(\widetilde{x}_t\) é a proposta de novo estado depois da filtragem não linear.
\end{itemize}

\subsubsection{3.4 Terceiro passo: misturar memória e atualizacao nova}\label{34-terceiro-passo-misturar-memuxf3ria-e-atualizacao-nova}
...
Essa e uma equação importante pedagogicamente. Ela mostra que o reservatório não "apaga" o passado a cada passo. O parametro \(\alpha\) controla a velocidade da atualizacao:
...
\item
  \(W_{res}\) modela a recorrencia interna do reservatório;
\item
  \(W_{in}\) projeta a entrada para o espaco de estados;
\item
  \(b\) desloca a ativacao;
\item
  \(\alpha\) controla o equilibrio entre memória e atualizacao.
\end{itemize}
