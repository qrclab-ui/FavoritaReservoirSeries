...
\tightlist
\item
  \(u_t \in \mathbb{R}^d\) é o vetor de entrada no tempo \(t\);
\item
  \(x_t \in \mathbb{R}^N\) é o estado do reservatório;
\item
  \(W_{res}\) é a matriz recorrente interna;
\item
  \(W_{in}\) projeta a entrada para o espaco do reservatório;
\item
  \(b\) é um vetor de bias;
\item
  \(\alpha\) é o \texttt{leak\_rate}.
\end{itemize}
