...
\subsection{3. Construindo o modelo mínimo de RC}\label{3-construindo-o-modelo-muxednimo-de-rc}

Em muitos textos de RC, a equação do reservatório aparece pronta, em uma única linha. Para quem está vendo o tema pela primeira vez, isso costuma ser brusco demais. Aqui vamos fazer o caminho inverso: primeiro identificamos os ingredientes da atualização, depois construímos o estado candidato, e só ao final escrevemos a equação completa.

\subsubsection{3.1 Os ingredientes da atualização}\label{31-os-ingredientes-da-atualizauxe7uxe3o}

Em cada instante \(t\), o reservatório recebe uma entrada \(u_t\) e mantém internamente um estado \(x_{t-1}\) vindo do passo anterior. O objetivo da atualização é responder à pergunta:

\begin{quote}
dado o que entrou agora e dado o que o sistema ainda "lembra" do passado, qual deve ser o novo estado interno?
\end{quote}
...
\item
  \(W_{in}\), a matriz que projeta a entrada para o espaço do reservatório;
\item
  \(W_{res}\), a matriz recorrente interna, que mistura os componentes do próprio estado;
\item
  \(b\), um vetor de bias;
\item
  \(\alpha \in (0,1]\), o \texttt{leak\_rate}, que controlará quanto do estado novo realmente entra.
\end{itemize}

Antes de pensar em "novo estado", vale notar que o reservatório olha para duas fontes de informação ao mesmo tempo:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  a entrada atual, que chega por \(W_{in} u_t\);
\item
  a memória interna, que chega por \(W_{res} x_{t-1}\).
\end{enumerate}

\subsubsection{3.2 Primeiro passo: combinar passado e entrada}\label{32-primeiro-passo-combinar-passado-e-entrada}

O primeiro passo é formar uma ativação bruta, que ainda não é o novo estado final. Chamaremos essa quantidade de pré-ativação:

\[
z_t = W_{res} x_{t-1} + W_{in} u_t + b.
\]

Essa expressão deve ser lida com calma:

\begin{itemize}
\tightlist
\item
  \(W_{res} x_{t-1}\) leva a informação do passado para o presente;
\item
  \(W_{in} u_t\) injeta a entrada atual no reservatório;
\item
  \(b\) desloca a resposta de cada unidade.
\end{itemize}

O vetor \(z_t\) representa, portanto, a resposta linear bruta do sistema antes de qualquer limitação ou filtragem.

\subsubsection{3.3 Segundo passo: obter o estado candidato}\label{33-segundo-passo-obter-o-estado-candidato}

Se usássemos diretamente \(z_t\) como novo estado, o modelo permaneceria linear demais e os valores internos poderiam crescer sem controle. Por isso, aplicamos uma não linearidade componente a componente:
...
O vetor \(\widetilde{x}_t\) é chamado aqui de \textbf{estado candidato}. Ele representa a resposta que o reservatório produziria se reagisse imediatamente ao passo atual. A função \(\tanh(\cdot)\) comprime os valores para o intervalo \((-1,1)\), introduz não linearidade e ajuda a manter a dinâmica interna em uma faixa estável.
...
\tightlist
\item
  \(z_t\) é a ativação bruta;
\item
  \(\widetilde{x}_t\) é a proposta de novo estado depois da filtragem não linear.
\end{itemize}

\subsubsection{3.4 Terceiro passo: misturar memória e atualização nova}\label{34-terceiro-passo-misturar-memuxf3ria-e-atualizauxe7uxe3o-nova}
...
Essa é uma equação importante pedagogicamente. Ela mostra que o reservatório não "apaga" o passado a cada passo. O parâmetro \(\alpha\) controla a velocidade da atualização:

\begin{itemize}
\tightlist
\item
  se \(\alpha\) é pequeno, o sistema preserva mais memória e muda devagar;
\item
  se \(\alpha\) é grande, o sistema reage mais ao presente e muda mais rápido.
\end{itemize}

\subsubsection{3.5 A equação completa do reservatório}\label{35-a-equauxe7uxe3o-completa-do-reservatuxf3rio}

Agora sim podemos juntar os passos anteriores. Substituindo o estado candidato na equação de mistura, chegamos à dinâmica completa:
...
\tightlist
\item
  \(u_t\) é a entrada atual;
\item
  \(x_{t-1}\) é a memória interna trazida do passo anterior;
\item
  \(W_{res}\) modela a recorrência interna do reservatório;
\item
  \(W_{in}\) projeta a entrada para o espaço de estados;
\item
  \(b\) desloca a ativação;
\item
  \(\alpha\) controla o equilíbrio entre memória e atualização.
\end{itemize}

No código, essa construção aparece de forma quase literal em \texttt{EchoStateReservoir.step()}:
...
A correspondência entre teoria e implementação fica mais fácil de ler quando usamos os mesmos três passos:
...
\item
  \texttt{self.state\ =\ ...} corresponde à atualização final de \(x_t\).
\end{enumerate}

\subsubsection{3.6 Como a previsão sai do estado interno}\label{36-como-a-previsuxe3o-sai-do-estado-interno}
...
No projeto, \texttt{\textbackslash{}phi\_t} é construída por \texttt{\_feature\_vector()} e \texttt{W\_out} é ajustado via regressão Ridge.

\subsection{4. O que é treinado e o que não é treinado}\label{4-o-que-uxe9-treinado-e-o-que-nuxe3o-uxe9-treinado}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Exemplo dos sinais de entrada sequenciais usados pelo RC.}]{computational_results_20260402_222902/sequential_inputs_example.png}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Heatmap dos estados internos do reservatório clássico no recorte adotado no Favorita.}]{computational_results_20260402_222902/rc_state_heatmap.png}}
